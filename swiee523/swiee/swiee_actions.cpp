...
bool SwieeApp::performToggleAction(bool on, QAction *Action,
...
// -----------------------------------------------------------------------
void SwieeApp::slotEditPaste(bool on)
{
...
// -----------------------------------------------------------------------
void SwieeApp::slotInsertEntity ()
{
  TextDoc * Doc = (TextDoc *) DocumentTab->currentPage ();
  Doc->viewport()->setFocus ();
  //TODO Doc->clearParagraphBackground (Doc->tmpPosX);
...
void SwieeApp::slotInsertEquation(bool on)
{
  editText->setHidden(true); // disable text edit of component property
  MouseReleaseAction = 0;
  MouseDoubleClickAction = 0;
...
void SwieeApp::slotInsertGround(bool on)
{
  editText->setHidden(true); // disable text edit of component property
  MouseReleaseAction = 0;
  MouseDoubleClickAction = 0;
...
void SwieeApp::slotInsertPort(bool on)
{
  editText->setHidden(true); // disable text edit of component property
  MouseReleaseAction = 0;
  MouseDoubleClickAction = 0;
...
void SwieeApp::slotDistribHoriz()
{
  editText->setHidden(true); // disable text edit of component property
...
void SwieeApp::slotCenterHorizontal()
{
  editText->setHidden(true); // disable text edit of component property
...
void SwieeApp::slotCenterVertical()
{
  editText->setHidden(true); // disable text edit of component property
...
void SwieeApp::slotSelectMarker()
{
  editText->setHidden(true); // disable text edit of component property
...
void SwieeApp::editFile(const QString& File)
{
    if (SwieeSettings.Editor.toLower() == "qucs" | SwieeSettings.Editor.isEmpty())
    {
        // The Editor is 'qucs' or empty, open a net document tab
        if (File.isEmpty()) {
            SwieeApp::slotTextNew();
        }
        else
        {
            editText->setHidden(true); // disable text edit of component property
    ...
    else
    {
      ...
      if (SwieeSettings.Editor.toLower().contains("qucsedit")) {
  ...
  prog = "qucsedit";
        ...
        QFileInfo editor(SwieeSettings.BinDir + prog);
        prog = QDir::toNativeSeparators(editor.canonicalFilePath());
      }
      else { // user defined editor
          QFileInfo editor(SwieeSettings.Editor);
          prog = QDir::toNativeSeparators(editor.canonicalFilePath());
      }
      ...
      QProcess *SwieeEditor = new QProcess();
      QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
      env.insert("PATH", env.value("PATH") );
      SwieeEditor->setProcessEnvironment(env);

      qDebug() << "Command: " << prog << args.join(" ");

      SwieeEditor->start(prog, args);

      if( !SwieeEditor->waitForStarted(1000) ) {
        QMessageBox::critical(this, tr("Error"), tr("Cannot start text editor! \n\n%1").arg(prog));
        delete SwieeEditor;
        return;
      }
      qDebug() << SwieeEditor->readAllStandardError();

      // to kill it before qucs ends
      connect(this, SIGNAL(signalKillEmAll()), SwieeEditor, SLOT(kill()));
    }
}

// ------------------------------------------------------------------------
// Is called to show the output messages of the last simulation.
void SwieeApp::slotShowLastMsg()
{
  editFile(SwieeSettings.SwieeHomeDir.filePath("log.txt"));
}

// ------------------------------------------------------------------------
// Is called to show the netlist of the last simulation.
void SwieeApp::slotShowLastNetlist()
{
  editFile(SwieeSettings.SwieeHomeDir.filePath("netlist.txt"));
}

// ------------------------------------------------------------------------
// Is called to start the text editor.
void SwieeApp::slotCallEditor()
{
  editFile(QString(""));
}
...
void SwieeApp::slotCallFilter()
{
  ...
  QProcess *SwieeFilter = new QProcess();

  SwieeFilter->setWorkingDirectory(SwieeSettings.BinDir);
  SwieeFilter->start(prog);

  prog = QDir::toNativeSeparators(SwieeSettings.BinDir+prog);
  qDebug() << "Command :" << prog;

  if( !SwieeFilter->waitForStarted(1000) ) {
  ...
  // to kill it before qucs ends
  connect(this, SIGNAL(signalKillEmAll()), SwieeFilter, SLOT(kill()));
}

// ------------------------------------------------------------------------
// Is called to start the transmission line calculation program.
void SwieeApp::slotCallLine()
{
  QString prog;
#ifdef __MINGW32__
  prog = "qucstrans.exe";
  ...
  prog = "qucstrans";
#endif
  QProcess *SwieeLine = new QProcess();

  SwieeLine->setWorkingDirectory(SwieeSettings.BinDir);
  SwieeLine->start(prog);

  prog = QDir::toNativeSeparators(SwieeSettings.BinDir+prog);
  qDebug() << "Command :" << prog;

  if( !SwieeLine->waitForStarted(1000) ) {
    QMessageBox::critical(this, tr("Error"),
  ...
  // to kill it before qucs ends
  connect(this, SIGNAL(signalKillEmAll()), SwieeLine, SLOT(kill()));
}
...
void SwieeApp::slotCallLibrary()
{
  ...
  SwieeLibrary->setWorkingDirectory(SwieeSettings.BinDir);
  SwieeLibrary->start(prog);

  prog = QDir::toNativeSeparators(SwieeSettings.BinDir+prog);
  qDebug() << "Command :" << prog;

  if( !SwieeLibrary->waitForStarted(1000) ) {
  ...
  // to kill it before qucs ends
  connect(this, SIGNAL(signalKillEmAll()), SwieeLibrary, SLOT(kill()));
}

// --------------------------------------------------------------
// Is called to show a dialog for creating matching circuits.
void SwieeApp::slotCallMatch()
{
...
void SwieeApp::slotCallAtt()
{
  ...
  QProcess *SwieeAtt = new QProcess();

  SwieeAtt->setWorkingDirectory(SwieeSettings.BinDir);
  SwieeAtt->start(prog);

  prog = QDir::toNativeSeparators(SwieeSettings.BinDir+prog);
  qDebug() << "Command :" << prog;

  if( !SwieeAtt->waitForStarted(1000) ) {
    QMessageBox::critical(this, tr("Error"),
  ...
  // to kill it before qucs ends
  connect(this, SIGNAL(signalKillEmAll()), SwieeAtt, SLOT(kill()));
}
// ------------------------------------------------------------------------
// Is called to start the resistor color code calculation program.
void SwieeApp::slotCallRes()
{
  QString prog;
#ifdef __MINGW32__
  ...
  QProcess *SwieeRes = new QProcess();

  SwieeRes->setWorkingDirectory(SwieeSettings.BinDir);
  SwieeRes->start(prog);

  prog = QDir::toNativeSeparators(SwieeSettings.BinDir+prog);
  qDebug() << "Command :" << prog;

  if( !SwieeRes->waitForStarted(1000) ) {
    QMessageBox::critical(this, tr("Error"),
  ...
  // to kill it before qucs ends
  connect(this, SIGNAL(signalKillEmAll()), SwieeRes, SLOT(kill()));
}
// --------------------------------------------------------------
void SwieeApp::slotHelpIndex()
{
  showHTML("index.html");
}

// --------------------------------------------------------------
void SwieeApp::slotGettingStarted()
{
  showHTML("start.html");
}

// --------------------------------------------------------------
void SwieeApp::showHTML(const QString& Page)
{
  ...
  QProcess *SwieeHelp = new QProcess();

  SwieeHelp->setWorkingDirectory(SwieeSettings.BinDir);
  SwieeHelp->start(com.join(" "));

  qDebug() << "Command :" << com.join(" ");
  if( !SwieeHelp->waitForStarted(1000) ) {
  ...
  // to kill it before qucs ends
  connect(this, SIGNAL(signalKillEmAll()), SwieeHelp, SLOT(kill()));
}

// ---------------------------------------------------------------------
// Is called when the find action is activated.
void SwieeApp::slotEditFind()
{
...
void SwieeApp::slotEditFindAgain()
{
  SearchDia->searchText(true, 1);
}

// --------------------------------------------------------------
void SwieeApp::slotChangeProps()
{
  QWidget *Doc = DocumentTab->currentPage();
  if(Doc->inherits("QPlainTextEdit")) {
    ((TextDoc*)Doc)->viewport()->setFocus();
...
// --------------------------------------------------------------
void SwieeApp::slotAddToProject()
{
  editText->setHidden(true); // disable text edit of component property
  ...
  QStringList List = QFileDialog::getOpenFileNames(this, tr("Select files to copy"),
    lastDir.isEmpty() ? QString(".") : lastDir, SwieeFileFilter);
  ...
  while(it != FileList.end()) {
    Info.setFile(*it);
    origFile.setName(*it);
    destFile.setName(SwieeSettings.SwieeWorkDir.absPath() +
...
void SwieeApp::slotApplyCompText()
{
  QString s;
  QFont f = SwieeSettings.font;
  Schematic *Doc = (Schematic*)DocumentTab->currentPage();
...
void SwieeApp::slotResizePropEdit(const QString& t)
{
...
// -----------------------------------------------------------
void SwieeApp::slotExportGraphAsCsv()
{
  editText->setHidden(true); // disable text edit of component property
...
// ----------------------------------------------------------
void SwieeApp::slotCreatePackage()
{
  editText->setHidden(true); // disable text edit of component property
...
// ----------------------------------------------------------
void SwieeApp::slotExtractPackage()
{
  editText->setHidden(true); // disable text edit of component property
...
void SwieeApp::slotOpenRecent(int num)
{
    //qDebug()<<SwieeSettings.RecentDocs.at(num);
    gotoPage(SwieeSettings.RecentDocs.at(num));
    updateRecentFilesList(SwieeSettings.RecentDocs.at(num));
    slotUpdateRecentFiles();
}

void SwieeApp::slotUpdateRecentFiles()
{
    ...
    QString file;
    foreach (file,SwieeSettings.RecentDocs) {
...
void SwieeApp::slotClearRecentFiles()
{
    SwieeSettings.RecentDocs.clear();
    slotUpdateRecentFiles();
}


/*!
 * \brief SwieeApp::slotLoadModule launches the dialog to select dynamic modueles
 */
void SwieeApp::slotLoadModule()
{
    ...
    QDir projDir = SwieeSettings.SwieeWorkDir.absolutePath();
...
/*!
 * \brief SwieeApp::slotBuildModule runs admsXml, C++ compiler to build library
 *
 * Run the va2cpp
 * Run the cpp2lib
 *
...
void SwieeApp::slotBuildModule()
{
    ...
    QDir prefix = QDir(SwieeSettings.BinDir+"../");

    QDir include = QDir(SwieeSettings.BinDir+"../include/qucs-core");

    QString workDir = SwieeSettings.SwieeWorkDir.absolutePath();
    ...
    QString admsXml = SwieeSettings.AdmsXmlBinDir.canonicalPath();
