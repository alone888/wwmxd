...
AnimateDiagram::AnimateDiagram(int _cx, int _cy) : Diagram(_cx, _cy)
{
...
AnimateDiagram::~AnimateDiagram()
{
}

// ------------------------------------------------------------
void AnimateDiagram::calcCoordinate(double* &xD, double* &yD, double* &,
...
// --------------------------------------------------------------
void AnimateDiagram::calcLimits()
{
  int i;
  double a, b, c;
...
// --------------------------------------------------------------
int AnimateDiagram::calcDiagram()
{
  Lines.clear();
  Texts.clear();
  Arcs.clear();
  ...
  QFontMetrics  metrics(((Schematic*)SwieeMain->DocumentTab->currentPage())->font());   // get size of text
  y1 = SwieeSettings.font.pointSize() + 6;
...
// ------------------------------------------------------------
bool AnimateDiagram::insideDiagram(float x, float y)
{
  return (regionCode(x, y) == 0);
}

// ------------------------------------------------------------
void AnimateDiagram::clip(float* &p)
{
  rectClip(p);
}

// ------------------------------------------------------------
Diagram* AnimateDiagram::newOne()
{
  return new AnimateDiagram();
}

// ------------------------------------------------------------
Element* AnimateDiagram::info(QString& Name, char* &BitmapFile, bool getNewOne)
{
  Name = QObject::tr("Running");
  BitmapFile = (char *) "animate";

  if(getNewOne)  return new AnimateDiagram();
  return 0;
}

void AnimateDiagram::paintAD(ViewPainter *p)
{
	paint(p);

	// QTimer triggered repaint
	// draw moving points of all graphs
	p->Painter->setPen(QPen(Qt::red, Graphs.first()->Thick*p->PrintScale, Qt::SolidLine));
	p->Painter->setBrush(QBrush(Qt::red, Qt::SolidPattern));
	for(Graph *pg = Graphs.first(); pg != 0; pg = Graphs.next())
	{
		if(*(pg->ScrPoints+livePointNum*2) == GRAPHEND ) livePointNum = 0; // periodically moving
		p->drawOneCircleSymbol(cx, cy, pg->ScrPoints, livePointNum);
		livePointNum += liveSch->richDiagNum;
	}
}