                    ...
                    : QDialog(parent, 0, TRUE, Qt::WDestructiveClose)
{
  ...
  QString NameY, NameZ;
  if((Diag->Name == "Rect") || (Diag->Name == "Curve") || (Diag->Name == "Running")) {
    NameY = tr("left Axis");
    NameZ = tr("right Axis");
  }
  ...
  if(Diag->Name == "Tab") {
    ...
    if((Diag->Name=="Rect") || (Diag->Name=="PS") || (Diag->Name=="SP") || (Diag->Name=="Curve") || (Diag->Name=="Running")) {
  ...
  // Tab #2...........................................................
  int Row = 0;
  if(Diag->Name.at(0) != 'T' && Diag->Name != "RichText") {  // not tabular or timing diagram and not richtext (xuliang)
	  // while Running curve needs these settings, tab2 and tab3
    ...
    if((Diag->Name.left(4) == "Rect") || (Diag->Name == "Curve") || (Diag->Name == "Running")) {
    ...
    if(Diag->Name.left(4) != "Rect" && (Diag->Name != "Running"))   // cartesian 2D and 3D
      if(Diag->Name != "Curve") {
...
void DiagramDialog::slotTakeVar(QTableWidgetItem* Item)
{
  ...
  //if(s.isEmpty()) {
    ...
    if(Diag->Name != "Tab" && Diag->Name!="RichText") {
      if(Diag->Name != "Truth") {
...
{
  if(Diag->Name.at(0) != 'T' && Diag->Name != "RichText") {  // not tabular or timing, nor richtext
    ...
    if(Diag->Name.left(4) == "Rect" || (Diag->Name == "Running")) {
  ...
  //if( Diag->Name != "RichText" )
  for(Graph *pg = Graphs.first(); pg != 0; pg = Graphs.next())
    Diag->Graphs.append(pg);  // transfer the new graphs to diagram
  //else Diag->Graphs.append(Graphs.first ());
  Graphs.clear();
  Graphs.setAutoDelete(true);
	

	if(0 && Diag->Name == "RichText") // added by xuliang
	{
		Q3ScrollView *pp = (Q3ScrollView*)parent();
		QString picFileName = QFileDialog::getOpenFileName(this,GB2312("请选择场景"),"",("Image File(*.*)")) ;
		//QString fileName = tr("h:/20150426173456.jpg");
		if( picFileName != "" ) 
		{
			swieeLabel *newLabel = new swieeLabel("CHILD", pp->viewport());
			//QObject::connect(newLabel, SIGNAL(swieeLabelDoubleClicked()), this, SLOT(slotLabelDoubleClicked(QMouseEvent *e)));
			newLabel->setGeometry(QRect(20, 20, 200, 20)); // initially ignore width and height, xuliang
			QImage image ;
			image.load(picFileName) ;

			newLabel->setPixmap(QPixmap::fromImage(image));
			newLabel->resize(QSize(image.width(),image.height()));
			//ui->scrollImag->setWidget(newLabel);
			//newLabel->setText(GB2312("就是想测试一下嘛"));
			//newLabel->setMo
			newLabel->show();
			pp->addChild(newLabel);
			//pp->viewport()->insertChild(newLabel);

			((Schematic*)pp)->viewport()->update();
			((Schematic*)pp)->viewport()->repaint ();

			return;
		}
		else
		{
			//QGroupBox *qgl1 = new QGroupBox(pp->viewport() );
			swieeLabel *pl1=new swieeLabel("pl1",pp->viewport());
			//connect(pl1, SIGNAL(swieeLabelClicked()), pp, SLOT(((Schmatic *)pp)->slotLabelDoubleClicked(/*QMouseEvent *e*/)));
			//connect(pl1, SIGNAL(swieeLabelClicked()), pp, SLOT(contentsMousePressEvent(QMouseEvent *e)));
			//connect(pl1, SIGNAL(swieeLabelDoubleClicked()), pp->viewport(), SLOT(slotLabelDoubleClicked(/*QMouseEvent *e*/)));
			//pte->setGeometry(QRect(40, 40, 200, 40));
			//qgl1->setGeometry (30, 30, 220, 30);
			pl1->setGeometry (5, 5, 200, 20);
			//qgl1->insertChild(pl1);
			//pte->move();
			pl1->setText(tr("123456789"));
			//qgl1->show();
			pl1->show();
			
			//pp->addChild (qgl1,100,60);
			pp->addChild (pl1,100,100);
			((Schematic*)pp)->viewport()->update();
			((Schematic*)pp)->viewport()->repaint ();
  ...
  // for richtext, it seems:
  // Diag->loadVarData(defaultDataSet, Diag->Graphs.first());
  // then, points will be in Diag->Graphs.first()->cPointsX 
  // then, 
  Diag->loadGraphData(defaultDataSet); // THE OLD LINE, ONLY THIS ONE, xuliang
...
//  ((SwieeView*)parent())->viewport()->repaint();
