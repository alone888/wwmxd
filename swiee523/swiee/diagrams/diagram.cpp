...
Diagram::Diagram(int _cx, int _cy)
{
  ...
  livePointNum = 0;
  liveSch = (Schematic*)SwieeMain->DocumentTab->currentPage();
...
//      pos = -1;
  ...
  *pPos = '>';
  ...
  // it seems enough for richtext to return here, by xuliang
  //if(Name == "RichText") return 2;
...
void Diagram::createSmithChart(Axis *Axis, int Mode)
{
  ...
  if(Axis->up > 1.0) {  // draw circle with |r|=1 ?
    ...
    if(Below)  y = 4;
    else  y = y2-4-SwieeSettings.font.pointSize();
...
void Diagram::createPolarDiagram(Axis *Axis, int Mode)
{
  ...
  int phi, tPos;
  int tHeight = SwieeSettings.font.pointSize() + 5;
