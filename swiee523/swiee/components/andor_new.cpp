...
andor_new::andor_new()
{
  ...
  Description = QObject::tr ("andor_new verilog device");
...
Component * andor_new::newOne()
{
  andor_new * p = new andor_new();
  p->Props.getFirst()->Value = Props.getFirst()->Value; 
  p->recreate(0); 
  return p;
}

Element * andor_new::info(QString& Name, char * &BitmapFile, bool getNewOne)
{
	Name = QObject::tr("4x4 AndOr");
	BitmapFile = (char *) "z6";

  if(getNewOne) return new andor_new();
  return 0;
}


void andor_new::createSymbol()
{
...
QString andor_new::vhdlCode( int )
{
  QString s="";
...
QString andor_new::verilogCode( int )
{
  QString td = Props.at(1)->Value;        // delay time
