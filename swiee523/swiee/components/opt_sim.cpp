...
Optimize_Sim::Optimize_Sim()
{
  ...
  Texts.append(new Text(0, 0, Description, Qt::darkBlue, SwieeSettings.largeFontSize));
...
{
  Property* pp;
  QFile afile(SwieeSettings.SwieeHomeDir.filePath("asco_netlist.cfg"));
  ...
  QDir ExtractDir(SwieeSettings.SwieeHomeDir);
...
bool Optimize_Sim::createASCOnetlist()
{
  ...
  QFile infile(SwieeSettings.SwieeHomeDir.filePath("netlist.txt"));
  QFile outfile(SwieeSettings.SwieeHomeDir.filePath("asco_netlist.txt"));
...
bool Optimize_Sim::loadASCOout()
{
  ...
  QFile infile(SwieeSettings.SwieeHomeDir.filePath("asco_out.log"));
  if(!infile.open(QIODevice::ReadOnly)) return false;
  QTextStream instream(&infile);
  QString Line;
