...
{
  QFontMetrics  metrics(SwieeSettings.font);   // get size of text
  ...
  else
  {
    ...
    if (FileInfo.fileName () == FileName)
    {
        ...
        if (containingSchematic)
        {
            ...
            for (int i = 0; i < SwieeMain->spiceExtensions.count (); i++)
            {
                QFileInfo localFIleInfo (schematicFileInfo.canonicalPath ()
                                         + "/" + baseName + SwieeMain->spiceExtensions[i]);
    ...
    QMutex mutex;
    mutex.lock();
    QString hashsearchresult = SwieeMain->spiceNameHash.value(baseName);
    mutex.unlock();
...
bool SpiceFile::recreateSubNetlist(QString *SpiceFile, QString *FileName)
{
  ...
  QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
  env.insert("PATH", env.value("PATH") );
  SwieeConv->setProcessEnvironment(env);

  qDebug() << "Command:" << prog << com.join(" ");
//  SwieeConv->start(com.join(" "));
  SwieeConv->start(prog, com);
  ...
  connect(SwieeConv, SIGNAL(finished(int,QProcess::ExitStatus)), SLOT(slotExited()));

  if(SwieeConv->state()!=QProcess::Running&&
          SwieeConv->state()!=QProcess::Starting) {
    ErrText += QObject::tr("COMP ERROR: Cannot start SwieeConv!");
    return false;
  }
  (*outstream) << NetText;
  (*filstream) << NetText;
  ...
  QMessageBox *MBox = new QMessageBox(QObject::tr("Info"),
	       QObject::tr("Converting SPICE file \"%1\".").arg(*SpiceFile),
               ...
               QMessageBox::NoButton, QMessageBox::NoButton, 0, 0, true,
	       Qt::WStyle_DialogBorder | Qt::WDestructiveClose);
  connect(SwieeConv, SIGNAL(finished(int)), MBox, SLOT(close()));
  MBox->exec();
...
{
  ErrText += QString(SwieeConv->readAllStandardError());
}
...
void SpiceFile::slotGetNetlist()
{
  int i;
  QString s;
  NetLine += QString(SwieeConv->readAllStandardOutput());
...
{
  if (SwieeConv->exitStatus() != QProcess::NormalExit) {
    NetText = "";
  }
  else {
    if(makeSubcircuit) {
