...
SpiceDialog::SpiceDialog(SwieeApp* App_, SpiceFile *c, Schematic *d)
...
void SpiceDialog::slotButtBrowse()
{
    ...
    if(SwieeSettings.SwieeWorkDir.exists(Info.fileName()) &&
            SwieeSettings.SwieeWorkDir.absPath() == Info.dirPath(true)) s = Info.fileName();
    FileEdit->setText(s);
...
bool SpiceDialog::loadSpiceNetList(const QString& s)
{
    ...
    QFileInfo FileInfo(SwieeSettings.SwieeWorkDir, s);
    ...
    if (preprocessor != "none")
    {
        ...
        script = SwieeSettings.BinDir + script;
        ...
        QFile PrepFile;
        QFileInfo PrepInfo(SwieeSettings.SwieeWorkDir, s + ".pre");
        ...
        FileInfo = QFileInfo(SwieeSettings.SwieeWorkDir, s + ".pre");
    }

    // Now do the spice->swiee netlist conversion using the swieeconv program ...
    SwieeConv = new QProcess(this);
    ...
    connect(SwieeConv, SIGNAL(readyReadStandardOutput()), SLOT(slotGetNetlist()));
    connect(SwieeConv, SIGNAL(readyReadStandardError()), SLOT(slotGetError()));
    ...
    connect(SwieeConv, SIGNAL(finished(int, QProcess::ExitStatus)), MBox, SLOT(close()));

    SwieeConv->start(Program, Arguments);

//    if((SwieeConv->state() != QProcess::Starting) && (SwieeConv->state()!=QProcess::Running))
    if(!SwieeConv->Running)
    {
        QMessageBox::critical(this, tr("Error"),
                              tr("Cannot execute \"%1\".").arg(SwieeSettings.BinDir + "qucsconv" + executableSuffix));
        return false;
    }
    ...
    if(!Error.isEmpty())
        QMessageBox::critical(this, tr("SwieeConv Error"), Error);
...
{
    Error += QString(SwieeConv->readAllStandardError ());
}
...
void SpiceDialog::slotGetNetlist()
{
    ...
    int i;
    QString s;
    Line += QString(SwieeConv->readAllStandardOutput ());
...
{
    Doc->App->editFile(SwieeSettings.SwieeWorkDir.filePath(FileEdit->text()));
}
