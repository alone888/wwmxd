...
void ComponentDialog::slotBrowseFile()
{
  QString s = QFileDialog::getOpenFileName (
          this,
          tr("Select a file"),
          SwieeSettings.SwieeWorkDir.path(),
          tr("All Files")+" (*.*);;"
  ...
  if(!s.isEmpty()) {
    ...
    QFileInfo file(s);
    if(SwieeSettings.SwieeWorkDir.exists(file.fileName()) &&
       SwieeSettings.SwieeWorkDir.absPath() == file.dirPath(true)) s = file.fileName();
    edit->setText(s);
  }
...
{
  Doc->App->editFile(SwieeSettings.SwieeWorkDir.filePath(edit->text()));
}
