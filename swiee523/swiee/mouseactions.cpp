...
MouseActions::MouseActions(SwieeApp* App_)
{
  ...
  ComponentMenu = new Q3PopupMenu(SwieeMain);
...
void MouseActions::MMoveWire2(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SwieeMain->MouseDoubleClickAction = &MouseActions::MDoubleClickWire2;
  Doc->viewport()->update();
}
...
void MouseActions::MMoveMoving(Schematic *Doc, QMouseEvent *Event)
{
  ...
  MAx1 = MAx2;
  MAy1 = MAy2;
  SwieeMain->MouseMoveAction = &MouseActions::MMoveMoving2;
  SwieeMain->MouseReleaseAction = &MouseActions::MReleaseMoving;
...
void MouseActions::MMovePaste(Schematic *Doc, QMouseEvent *Event)
{
  ...
  drawn = true;
  SwieeMain->MouseMoveAction = &MouseActions::MMoveMoving2;
  SwieeMain->MouseReleaseAction = &MouseActions::MReleasePaste;
}
...
// FIXME #warning                      SwieeSettings.BGColor);
...
void MouseActions::rightPressMenu(Schematic *Doc, QMouseEvent *Event, float fX, float fY)
{
  ...
  while(true) {
    if(focusElement) {
      ...
      ComponentMenu->insertItem(
         QObject::tr("Edit Properties"), SwieeMain, SLOT(slotEditElement()));

      if((focusElement->Type & isComponent) == 0) break;
    }
    else {
      SwieeMain->symEdit->addTo(ComponentMenu);
      SwieeMain->fileSettings->addTo(ComponentMenu);
    }
    if(!SwieeMain->moveText->isOn())
      SwieeMain->moveText->addTo(ComponentMenu);
    break;
  }
  while(true) {
    if(focusElement)
      if(focusElement->Type == isGraph) break;
    if(!SwieeMain->onGrid->isOn())
      SwieeMain->onGrid->addTo(ComponentMenu);
    SwieeMain->editCopy->addTo(ComponentMenu);
    if(!SwieeMain->editPaste->isOn())
      SwieeMain->editPaste->addTo(ComponentMenu);
    break;
  }
  ...
  while (true) {
      if (focusElement) {
          if (focusElement->Type == isDiagram) {
              ...
              ComponentMenu->insertItem(QObject::tr("Export as image"), SwieeMain,
  ...
  if(!SwieeMain->editDelete->isOn())
    SwieeMain->editDelete->addTo(ComponentMenu);
  if(focusElement) if(focusElement->Type == isMarker) {
    ...
    ComponentMenu->insertItem(s, SwieeMain, SLOT(slotPowerMatching()));
    if( ((Marker*)focusElement)->pGraph->Var.left(2) == "S[" )
      ComponentMenu->insertItem(QObject::tr("2-port matching"), SwieeMain,
  ...
  do {
    if(focusElement) {
      ...
      if(focusElement->Type == isGraph) {
        SwieeMain->graph2csv->addTo(ComponentMenu);
        break;
      }
    }
    ComponentMenu->insertSeparator();
    if(focusElement) if(focusElement->Type & isComponent)
      if(!SwieeMain->editActivate->isOn())
        SwieeMain->editActivate->addTo(ComponentMenu);
    if(!SwieeMain->editRotate->isOn())
      SwieeMain->editRotate->addTo(ComponentMenu);
    if(!SwieeMain->editMirror->isOn())
      SwieeMain->editMirror->addTo(ComponentMenu);
    if(!SwieeMain->editMirrorY->isOn())
      SwieeMain->editMirrorY->addTo(ComponentMenu);

    // right-click menu to go into hierarchy
    if(focusElement) {
      if(focusElement->Type & isComponent)
	if(((Component*)focusElement)->Model == "Sub")
	  if(!SwieeMain->intoH->isOn())
	    SwieeMain->intoH->addTo(ComponentMenu);
    }
    // right-click menu to pop out of hierarchy
    if(!focusElement)
      if(!SwieeMain->popH->isOn())
	SwieeMain->popH->addTo(ComponentMenu);
  } while(false);
 ...
 //没搞懂
  ...
  switch(focusElement->Type)
  {
    case isPaintingResize:  // resize painting ?
      ...
      SwieeMain->MouseReleaseAction = &MouseActions::MReleaseResizePainting;
      SwieeMain->MouseMoveAction = &MouseActions::MMoveResizePainting;
      SwieeMain->MousePressAction = 0;
      SwieeMain->MouseDoubleClickAction = 0;
    ...
    case isDiagramResize:  // resize diagram ?
      ...
      SwieeMain->MouseReleaseAction = &MouseActions::MReleaseResizeDiagram;
    ...
    case isDiagramVScroll:
      ...
      switch(No)
      {
        ...
        case 2:  // move scroll bar with mouse cursor
          SwieeMain->MouseMoveAction = &MouseActions::MMoveScrollBar;
          SwieeMain->MousePressAction = 0;
          SwieeMain->MouseDoubleClickAction = 0;
          Doc->grabKeyboard();  // no keyboard inputs during move actions
    ...
    case isComponentText:  // property text of component ?
      ...
      MAx3 = No;
      SwieeMain->slotApplyCompText();
      // Update matching wire label highlighting
      Doc->highlightWireLabels ();
      return;

    case isNode:
      if (SwieeSettings.NodeWiring)
      {
        ...
        SwieeMain->MouseMoveAction = &MouseActions::MMoveWire2;
        SwieeMain->MousePressAction = &MouseActions::MPressWire2;
        SwieeMain->MouseReleaseAction = 0; // if function is called from elsewhere
        SwieeMain->MouseDoubleClickAction = 0;

        formerAction = SwieeMain->select; // to restore action afterwards
        SwieeMain->activeAction = SwieeMain->insWire;

        SwieeMain->select->blockSignals(true);
        SwieeMain->select->setOn(false);
        SwieeMain->select->blockSignals(false);

        SwieeMain->insWire->blockSignals(true);
        SwieeMain->insWire->setOn(true);
        SwieeMain->insWire->blockSignals(false);
        // Update matching wire label highlighting
        Doc->highlightWireLabels ();
        return;
      }
  ...
  else
  {
    ...
    Doc->setOnGrid(MAx1, MAy1);
    SwieeMain->MouseMoveAction = &MouseActions::MMoveMoving;
  }
  // Update matching wire label highlighting
  Doc->highlightWireLabels ();
}
...
//    if(Event->button() != Qt::LeftButton) return;
    ...
    MAy2 = 0;
    SwieeMain->MousePressAction = 0;
    SwieeMain->MouseReleaseAction = &MouseActions::MReleaseActivate;
    SwieeMain->MouseMoveAction = &MouseActions::MMoveSelect;
  }
  Doc->viewport()->update();
  drawn = false;
...
//    qDebug() << "+-+ got to switch:" << Comp->Name;
    ...
    switch(Event->button()) {
      case Qt::LeftButton :
		...
		App->updateNavigateDock(Doc);//add by wangyuanlong
		Comp->textSize(x2, y2);
...
//    qDebug() << "   => selElem = Comp;" << Comp->Name;
	...
	//if(Diag->Name == "RichText") return; // add by xuliang to test
    ...
    Doc->viewport()->repaint();
	// if richTextDiagram, then call Doc->liveTimer->start(500, false), added by xuliang
	if(Diag->Name == "RichText" ||Diag->Name == "Running")
	{
		// check if the live Timer already started
		if(!Doc->liveTimer->isActive())
		{
			Doc->liveTimer->start(100, false);
			Doc->liveTimerMaster = (Diagram *)Diag;
		}
		else
		{
			// to keep synchronous, just copy the existing timer master's point number;
			Diag->livePointNum = ((Diagram *)Doc->liveTimerMaster)->livePointNum ;
		}
	} // add-end by xuliang
...
void MouseActions::MPressWire2(Schematic *Doc, QMouseEvent *Event, float fX, float fY)
{
  ...
  case Qt::LeftButton :
    ...
    if(set1 & 2) {
      ...
      if(formerAction) {
        // ...restore old action
        SwieeMain->select->setOn(true);
      }
      else {
        // ...start a new wire
        SwieeMain->MouseMoveAction = &MouseActions::MMoveWire1;
        SwieeMain->MousePressAction = &MouseActions::MPressWire1;
        SwieeMain->MouseDoubleClickAction = 0;
      }
    }
...
void MouseActions::MPressMoveText(Schematic *Doc, QMouseEvent*, float fX, float fY)
{
  ...
  if(focusElement) {
    ...
    Doc->viewport()->update();
    drawn = false;
    SwieeMain->MouseMoveAction = &MouseActions::MMoveMoveText;
    SwieeMain->MouseReleaseAction = &MouseActions::MReleaseMoveText;
    Doc->grabKeyboard();  // no keyboard inputs during move actions
  }
}
...
void MouseActions::MPressZoomIn(Schematic *Doc, QMouseEvent*, float fX, float fY)
{
  ...
  SwieeMain->MouseMoveAction = &MouseActions::MMoveSelect;
  SwieeMain->MouseReleaseAction = &MouseActions::MReleaseZoomIn;
  Doc->grabKeyboard();  // no keyboard inputs during move actions
  Doc->viewport()->update();
  drawn = false;
}
...
void MouseActions::MReleaseSelect(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SwieeMain->MousePressAction = &MouseActions::MPressSelect;
  SwieeMain->MouseReleaseAction = &MouseActions::MReleaseSelect;
  SwieeMain->MouseDoubleClickAction = &MouseActions::MDoubleClickSelect;
  SwieeMain->MouseMoveAction = 0;   // no element moving
  Doc->highlightWireLabels ();
  Doc->viewport()->update();
  drawn = false;
}
...
void MouseActions::MReleaseActivate(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SwieeMain->MouseMoveAction = &MouseActions::MMoveActivate;
  SwieeMain->MousePressAction = &MouseActions::MPressActivate;
  SwieeMain->MouseReleaseAction = 0;
  SwieeMain->MouseDoubleClickAction = 0;
  Doc->highlightWireLabels ();
  Doc->viewport()->update();
  drawn = false;
...
void MouseActions::MReleasePaste(Schematic *Doc, QMouseEvent *Event)
{
  ...
  case Qt::LeftButton :
    ...
    SwieeMain->MouseMoveAction = &MouseActions::MMovePaste;
    SwieeMain->MousePressAction = 0;
    SwieeMain->MouseReleaseAction = 0;
    SwieeMain->MouseDoubleClickAction = 0;
...
void MouseActions::MReleaseMoveText(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SwieeMain->MouseMoveAction = &MouseActions::MMoveMoveTextB;
  SwieeMain->MouseReleaseAction = 0;
  Doc->releaseKeyboard();  // allow keyboard inputs again
...
void MouseActions::MReleaseZoomIn(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SwieeMain->MouseMoveAction = &MouseActions::MMoveZoomIn;
  SwieeMain->MouseReleaseAction = 0;
  Doc->releaseKeyboard();  // allow keyboard inputs again
}
...
{
  Doc->releaseKeyboard();  // allow keyboard inputs again
  SwieeMain->editText->setHidden(true);
  editElement(Doc, Event);

	/*/ add by xuliang to capture double click event
	float x = float(Event->pos().x())/Doc->Scale + float(Doc->ViewX1);
	float y = float(Event->pos().y())/Doc->Scale + float(Doc->ViewY1);

	swieeLabel *pl = (swieeLabel *)Doc->childAt(x, y);
	if( pl->name() == "pl1" )
	{
		pl->setGeometry (pl->x()+20,pl->y(),pl->width(), pl->height ());
	}
	/ end - xuliang adding*/
}
...
void MouseActions::MDoubleClickWire2(Schematic *Doc, QMouseEvent *Event)
{
  ...
  if(formerAction)
    SwieeMain->select->setOn(true);  // restore old action
  else {
    SwieeMain->MouseMoveAction = &MouseActions::MMoveWire1;
    SwieeMain->MousePressAction = &MouseActions::MPressWire1;
    SwieeMain->MouseDoubleClickAction = 0;
