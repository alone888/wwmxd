...
Schematic::Schematic(SwieeApp *App_, const QString& Name_)
    : SwieeDoc(App_, Name_)
{
  symbolMode = false;
  ...
  if(App) {
	...
	App->DocumentTab->currentPage()->setFont(QFont("SimSun",9)); // added by xuliang to control font size
...
// FIXME #warning removed those signals, crashes on it...
	...
	// added by xuliang to support live show
	richPointNum = 1; // initial value points the first data, note that the value in position 0 is the variable name
	richDiagNum = 0; // increase by 1 when a new richDiagram is added
	liveTimer = new QTimer(this);
	liveTimerMaster = 0;
	connect(liveTimer, SIGNAL(timeout()), this, SLOT(slotLivePaint()));
  } // of "if(App)"
}
...
void Schematic::setChanged(bool c, bool fillStack, char Op)
{
  ...
  if(symbolMode) {  // for symbol edit mode
    ...
    while(UndoSymbol.count() > SwieeSettings.maxUndo) { // "while..." because
  ...
  while(UndoStack.count() > SwieeSettings.maxUndo) { // "while..." because
...
// ---------------------------------------------------
// Saves this Swiee document. Returns the number of subcircuit ports.
int Schematic::save()
{
...
// new slot added by xuliang
void Schematic::slotLivePaint ()
{
	// 1. just repaint all components in current schematic in step I
	richDiagNum = 1;
	viewport()->repaint ();
	richDiagNum = 0;
	// 2. only check the live components in Q3PtrList<Diagram>   *Diagrams, DocDiags in step II
}