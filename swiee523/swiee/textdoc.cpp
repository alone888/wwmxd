 ...
 */
TextDoc::TextDoc(SwieeApp *App_, const QString& Name_) : SwieeDoc(App_, Name_), QPlainTextEdit()
{
  TextFont = QFont("Courier New");
  TextFont.setPointSize(SwieeSettings.font.pointSize()-1);
  ...
  //TODO (not supported) setUndoDepth(SwieeSettings.maxUndo);
  setLanguage (Name_);
  ...
  if(App) {
    ...
    setPaletteBackgroundColor(SwieeSettings.BGColor);
...
bool TextDoc::saveSettings (void)
{
  ...
  stream << "Textfile settings file, Swiee " PACKAGE_VERSION "\n"
 ...
 * See also SwieeApp::slotEditActivate
 */
