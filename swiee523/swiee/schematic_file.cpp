...
// -------------------------------------------------------------
// Creates a Swiee file format (without document properties) in the returning
...
bool Schematic::pasteFromClipboard(QTextStream *stream, Q3PtrList<Element> *pe)
{
  ...
  Line = Line.mid(17, Line.length()-18);
...
int Schematic::saveDocument()
{
  ...
  stream << "<Swiee Schematic " << PACKAGE_VERSION << ">\n";
...
//      QDir prefix = QDir(SwieeSettings.BinDir);
      ...
      qDebug() << "homedir"  << SwieeSettings.SwieeHomeDir.absolutePath();

      vaFile = SwieeSettings.SwieeWorkDir.filePath(fileBase()+".va");
...
//      env.insert("PATH", env.value("PATH") );
      ...
      QFile f1(SwieeSettings.SwieeWorkDir.filePath(fileBase()+"_props.json"));
      QFile f2(SwieeSettings.SwieeWorkDir.filePath(fileBase()+"_sym.json"));
      ...
      QFile f3(SwieeSettings.SwieeWorkDir.filePath(fileBase()+"_symbol.json"));
...
bool Schematic::loadDocument()
{
  ...
  while(!stream.atEnd()) {
    ...
    if(Line == "<Symbol>") {
      if ((Schematic*)SwieeMain != 0) // GUI running, load Symbols
    ...
    if ((Schematic*)SwieeMain != 0) { // GUI running, load Diagrams, Paintings
    if(Line == "<Diagrams>") {
    ...
    if(pc->Model.at(0) == '.') { // no simulations in subcircuits
      ...
      if(!isAnalog) {
        if (isVerilog) {
		  ...
		  (*it_name) += QString(" ") + ((*it_type).isEmpty() ? VHDL_SIGNAL_TYPE : (*it_type));
        }
      }
    }
  }
                              ...
                              QTextEdit *ErrText)
{
  ...
  if((allTypes & isAnalogComponent) == 0) {
    if(allTypes == 0) {
      ...
      // be a simulation within a SPICE file. Otherwise Swieeator will
      // output an error.
      isAnalog = true;
  ...
  if(allTypes & isAnalogComponent)
    ...
    stream << "--";
  stream << " Swiee " << PACKAGE_VERSION << "  " << DocName << "\n";
