 ...
 * \see SwieeApp::slotBuildModule() for the make output assignment.
 */
MessageDock::MessageDock(SwieeApp *App_): QWidget()
{
...
void MessageDock::slotCursor()
{
  ...
  // \todo set hightlight in SwieeDoc Verilog-A file?
  // move cursor? addt line number? highliht line number? set in focus
     ...
     * hightlings the line
     * SwieeApp::getDoc() //current should be a TextDoc
     */
//    SwieeDoc *foo =  SwieeMain->getDoc();
//    qWarning() << foo->DocName;
    ...
    if (gotoLine >= 0) {
        ...
        TextDoc * d = (TextDoc*)SwieeMain->getDoc();
