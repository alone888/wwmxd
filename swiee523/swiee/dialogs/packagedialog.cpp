			...
			: QDialog(parent_) //, 0, TRUE, Qt::WDestructiveClose)
{
  ...
  if(create_) {  // create or extract package ?
    ...
    // ...........................................................
    // insert all projects
    QStringList PrDirs = SwieeSettings.SwieeHomeDir.entryList("*", QDir::Dirs, QDir::Name);
...
{
  QFile File;
  QDir myDir(SwieeSettings.SwieeHomeDir.absPath() + QDir::separator() + "user_lib");
  QStringList Entries = myDir.entryList("*", QDir::Files, QDir::Name);
  QStringList::iterator it;
  for(it = Entries.begin(); it != Entries.end(); ++it) {
    File.setName(myDir.absFilePath(*it));
...
void PackageDialog::slotCreate()
{
  ...
  strcpy(Header, "Swiee package " PACKAGE_VERSION);
  ...
  while(i.hasNext()) {
    ...
    if(p->isChecked()) {
      ...
      s = SwieeSettings.SwieeHomeDir.absPath() + QDir::separator() + s;
  ...
  QMessageBox::information(this, tr("Info"),
          tr("Successfully created Swiee package!"));
  accept();
}
...
void PackageDialog::extractPackage()
{
  ...
  if(!PkgFile.open(QIODevice::ReadOnly)) {
    if(Info.extension().isEmpty()) s += ".swiee";
    PkgFile.setName(s);
    if(!PkgFile.open(QIODevice::ReadOnly)) {
  ...
  QDir currDir = SwieeSettings.SwieeHomeDir;
  QString Version;
  Q_UINT16 Checksum;
  ...
  QByteArray Content = PkgFile.readAll();
  if(strncmp(Content.data(), "Swiee package ", 13) != 0) {
...
int PackageDialog::extractLibrary(QFile& PkgFile, Q_UINT32 Count)
{
  ...
  p = Content.data();
  QFile File(SwieeSettings.SwieeHomeDir.absPath() +
