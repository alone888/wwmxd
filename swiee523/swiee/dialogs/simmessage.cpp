...
{
  //setWindowTitle(tr("Swiee Simulation Messages"));
  setWindowTitle(GB2312("仿真信息"));
  SwieeDoc *Doc;
  DocWidget = w;
  if(DocWidget->inherits("QTextEdit"))
    Doc = (SwieeDoc*) ((TextDoc*)DocWidget);
  else
    Doc = (SwieeDoc*) ((Schematic*)DocWidget);
  ...
  QGroupBox *Group1 = new QGroupBox(GB2312("仿真进度:"));
  ...
  QLabel *progr = new QLabel(GB2312("仿真进度:"));
...
//  SimProgress->setPercentageVisible(false);
  ...
  QGroupBox *Group2 = new QGroupBox(GB2312("错误和警告:"));
  ...
  Display = new QPushButton(GB2312("显示仿真结果"));
  ...
  Abort = new QPushButton(GB2312("中止仿真"));
...
bool SimMessage::startProcess()
{
  ...
  Abort->setText(GB2312("中止仿真"));
  Display->setDisabled(true);

  //QString txt = tr("Starting new simulation on %1 at %2").
  QString txt = GB2312("仿真开始时间： %1 at %2").
 ...
 /* On Qt4 it shows as running even before we .start it. FIXME*/
  if(SimProcess.state()==QProcess::Running ||SimProcess.state()==QProcess::Starting) {
	...
	ErrText->insert(GB2312("错误: 仿真器正在运行!"));
    FinishSimulation(-1);
    return false;
  }
  ...
  if(!DocWidget->inherits("QTextEdit")) {
    ...
    if(SimPorts < -5) {
	  ...
	  ErrText->insert(GB2312("错误: 无法仿真文本文件!"));
      FinishSimulation(-1);
      return false;
    }
  }
...
// ---------------------------------------------------
// Converts a spice netlist into Swiee format and outputs it.
...
// FIXME #warning SPICE section below not being covered?
  ...
  QString prog;
  QStringList com;
  prog = SwieeSettings.BinDir + "qucsconv" + executableSuffix;
  if(makeSubcircuit)
    com << "-g" << "_ref";
  ...
  if(FileName.find(QDir::separator()) < 0)  // add path ?
    SpiceFile.setName(SwieeSettings.SwieeWorkDir.path() + QDir::separator() + FileName);
  else
  ...
  qDebug() << "start SwieeConv" << prog << com.join(" ");
  SimProcess.start(prog, com);

  if(!SimProcess.Running) {
    ErrText->insert(tr("SIM ERROR: Cannot start SwieeConv!"));
    FinishSimulation(-1);
    return;
  }
...
void SimMessage::startSimulator()
{
  ...
  QString SimPath = QDir::convertSeparators (SwieeSettings.SwieeHomeDir.absPath());
#ifdef __MINGW32__
  QString SwieeDigiLib = "swieedigilib.bat";
  QString SwieeDigi = "swieedigi.bat";
  QString SwieeVeri = "swieeveri.bat";
#else
  QString SwieeDigiLib = "swieedigilib";
  QString SwieeDigi = "swieedigi";
  QString SwieeVeri = "swieeveri";
#endif
  SimOpt = NULL;
  ...
  if(DocWidget->inherits("QTextEdit")) {
...
#endif
      Program = pathName(SwieeSettings.BinDir + SwieeDigi);
      Arguments  << SwieeSettings.SwieeHomeDir.filePath("netlist.txt")
                 << DataSet << SimTime << pathName(SimPath)
                 << pathName(SwieeSettings.BinDir) << libs;
    }
    // Module.
    else {
      ...
      QString dir = QDir::convertSeparators (SwieeSettings.SwieeHomeDir.path());
      QDir vhdlDir(dir);
	...
	return;
      ...
      destFile.close();
      Program = pathName(SwieeSettings.BinDir + SwieeDigiLib);
      Arguments << SwieeSettings.SwieeHomeDir.filePath("netlist.txt")
                << pathName(SimPath)
                << entity
                << lib;
  ...
  // Simulate schematic window.
    ...
    if(SimPorts < 0) {
      ...
      if (! Module::vaComponents.isEmpty()) {
          ...
          /*! Only pass modules to Swieeator that are indeed used on
           * the schematic,it might be the case that the user loaded the icons,
           * but did not compiled the module. Swieeator will not find the library.
           *
           * Check if used symbols have corresponing lib before running
           * Swieeator? Need to search on the netlis.txt? Is there other data
          ...
          */
          ...
          if (! usedComponents.isEmpty()) {
            ...
            //Arguments << "-p" << SwieeSettings.SwieeWorkDir.absolutePath()
            //          << "-m" << usedComponents;
            ...
            if (!NetlistFile.open(QFile::Append | QFile::Text))
               ...
               out << "\n";
               out << "# --path=" << SwieeSettings.SwieeWorkDir.absolutePath() << "\n";
      ...
      if((SimOpt = findOptimization((Schematic*)DocWidget))) {
        ...
        Program = SwieeSettings.AscoBinDir.canonicalPath();
        Program = QDir::toNativeSeparators(Program+"/"+"asco"+QString(executableSuffix));
        Arguments << "-swiee" << SwieeSettings.SwieeHomeDir.filePath("asco_netlist.txt")
                  << "-o" << "asco_out";
      }
      else {
        Program = SwieeSettings.BinDir + "qucsator" + executableSuffix;
        Arguments << "-b" << "-g" << "-i"
                  << SwieeSettings.SwieeHomeDir.filePath("netlist.txt")
                  << "-o" << DataSet;
      }
    }
    else {
      if (isVerilog) {
          Program = QDir::toNativeSeparators(SwieeSettings.BinDir + SwieeVeri);
          Arguments << QDir::toNativeSeparators(SwieeSettings.SwieeHomeDir.filePath("netlist.txt"))
                    << DataSet
                    << SimTime
                    << QDir::toNativeSeparators(SimPath)
                    << QDir::toNativeSeparators(SwieeSettings.BinDir)
                    << "-c";
      } else {
/// \todo \bug error: unrecognized command line option '-Wl'
#ifdef __MINGW32__
    Program = QDir::toNativeSeparators(pathName(SwieeSettings.BinDir + SwieeDigi));
    Arguments << QDir::toNativeSeparators(SwieeSettings.SwieeHomeDir.filePath("netlist.txt"))
              << DataSet
              << SimTime
              << QDir::toNativeSeparators(SimPath)
              << QDir::toNativeSeparators(SwieeSettings.BinDir) << "-Wall" << "-c";
#else
    Program = QDir::toNativeSeparators(pathName(SwieeSettings.BinDir + SwieeDigi));
    Arguments << SwieeSettings.SwieeHomeDir.filePath("netlist.txt")
              << DataSet << SimTime << pathName(SimPath)
		      << pathName(SwieeSettings.BinDir) << "-Wall" << "-c";
  ...
  // insert Swiee bin dir, so ASCO can find qucsator
...
void SimMessage::FinishSimulation(int Status)
{
  ...
  if(Status == 0) {
	...
	QString txt = GB2312("仿真结束时间： %1 at %2").
	...
	QString txt = GB2312("仿真出错，时间为 %1 at %2").
      arg(d.toString("ddd dd. MMM yyyy")).
      arg(t.toString("hh:mm:ss:zzz"));
    ProgText->insert("\n" + txt + "\n" + tr("Aborted.") + "\n");
  }

  QFile file(SwieeSettings.SwieeHomeDir.filePath("log.txt"));  // save simulator messages
  ...
  if(Status == 0) {
    if(SimOpt) { // save optimization data
      QFile ifile(SwieeSettings.SwieeHomeDir.filePath("asco_out.dat"));
      QFile ofile(DataSet);
