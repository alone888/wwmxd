...
LibraryDialog::LibraryDialog(SwieeApp *App_, QTreeWidgetItem *SchematicList)
			: QDialog(App_)
{
  App = App_;
...
void LibraryDialog::slotSave()
{
  ...
  Stream << "<Swiee Library " PACKAGE_VERSION " \""
	 << NameEdit->text() << "\">\n\n";
  ...
  for (int i=0; i < SelectedNames.count(); i++) {
    ...
    Schematic *Doc = new Schematic(0, SwieeSettings.SwieeWorkDir.filePath(SelectedNames[i]));
    ...
    ErrText->insert("\n");
    ErrText->insert(tr("Creating Swiee netlist.\n"));
