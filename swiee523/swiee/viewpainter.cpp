...
// -------------------------------------------------------------
// draw one circle for moving point, added by xuliang
void ViewPainter::drawOneCircleSymbol(int x0, int y0, float *pp, int n)
{
  int d;
  float z, DX_, DY_;
  if(*pp < 0)
...
//while(*pp > GRAPHEND) {
//    if(*pp >= 0) {
      z = DX_ + (*(pp+(2*n))-4.0)*Scale;
      x0 = TO_INT(z);
      z = DY_ - (*(pp+(2*n+1))+4.0)*Scale;
      y0 = TO_INT(z);
      Painter->drawEllipse(x0, y0, d, d);
//    }
//    else  pp++;
